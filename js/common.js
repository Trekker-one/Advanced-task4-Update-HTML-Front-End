...
var cart_service_url = 'http://localhost:32601/cart/';
// Value depends on the deployment YAML config
...
function checkout(e) {
	...
	console.log(sessionStorage.products);
	var products = JSON.parse(sessionStorage.products);
	// generate unique orderId and cartId
	var _orderId = guidGenerator();
	var _cartId = guidGenerator();
	var _productIds = [];
	var _productPrices = [];
	var _total = 0;
	products.forEach(function (product){
		_productIds.push(product.productId);
		_productPrices.push(product.price);
		_total += product.qty;
	});
	...
	axios.post(cart_service_url, cart_order_body)
		.then(response => {
